...
# conditionally set input layer and quantum layer depending on the dataset
    ...
    if datatype == "COLORS" or datatype == "CIFAR10" or datatype == "COLORS_SHAPE":
        ...
        x_qconv1 = U1_circuit(n_kernels=5, n_input_channels=3,activation='relu',datatype=datatype,
                      name='CO_U1_QCNN')(x_input)

    x_maxpool = tf.keras.layers.MaxPooling2D(pool_size=(2, 2))(x_qconv1)
    x_flatten = tf.keras.layers.Flatten()(x_maxpool)
...
def CO_U2_QCNN_model(datatype,classes):
    ...
    if datatype == "COLORS" or datatype == "CIFAR10" or datatype == "COLORS_SHAPE":
        ...
        x_qconv1 = U2_circuit(n_kernels=5, n_input_channels=3,activation=tf.keras.layers.Activation('relu'),datatype=datatype,
                      name='CO_U2_QCNN')(x_input)
    x_maxpool = tf.keras.layers.MaxPooling2D(pool_size=(2, 2))(x_qconv1)
    x_flatten = tf.keras.layers.Flatten()(x_maxpool)
