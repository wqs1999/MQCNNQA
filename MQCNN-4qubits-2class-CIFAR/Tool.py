...
def filter_classes(target):
    return target in [6, 8]  # bird: 2, deer: 4
def DataLoad(batch_size=32, num_workers=4, input_size=32):
    ...
    trainset = tv.datasets.CIFAR10(  # PyTorch提供的CIFAR-10数据集的类，用于加载CIFAR-10数据集。
        ...
        transform=transform,
        target_transform=filter_classes)  # 设置数据集的预处理方式。
    ...
    testset = tv.datasets.CIFAR10(
        ...
        download=True,
        transform=transform,
        target_transform=filter_classes)
...
import copy
def filter_frog_ship(target):
    # 只选择frog(5)和ship(8)类别
    return target == 5 or target == 8
...
def DataLoader_1280(batch_size=32, num_workers=4, input_size=32, Train_num=640, Test_num=100, category=10):
    ...
    # 为每个类别随机选择1280个索引
    selected_indices = {i: np.random.choice(class_indices[i], Train_num, replace=False) for i in [0, 1]}
    ...
    # 为每个类别随机选择100个索引
    selected_indices = {i: np.random.choice(class_indices[i], Test_num, replace=False) for i in [0, 1]}
    ...
    test_loader = DataLoader(
        ...
        batch_size=Test_num*category,  # 每个小批量(batch)的大小是4，即每次会加载4张图片进行训练。
        shuffle=True,  # 在每个epoch训练开始前，会打乱训练集中数据的顺序，以增加训练效果。
        num_workers=num_workers,
        pin_memory=True)
    # 准备存储选取的索引
