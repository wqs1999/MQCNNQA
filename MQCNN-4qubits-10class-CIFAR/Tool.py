...
def DataLoader_1280(batch_size=32, num_workers=4, input_size=32, Train_num=640, Test_num=100, category=10):
    ...
    # 为每个类别随机选择1280个索引
    selected_indices = {i: np.random.choice(class_indices[i], Train_num, replace=False) for i in range(category)}
    ...
    # 为每个类别随机选择100个索引
    selected_indices = {i: np.random.choice(class_indices[i], Test_num, replace=False) for i in range(category)}
